...
class TemplateTemplate extends fbi_1.Template {
    ...
    checking() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const dirExist = yield this.fs.pathExists(path_1.join(this.targetDir, 'templates', template.id));
            const fileExist = yield this.fs.pathExists(path_1.join(this.targetDir, this.features.typescript ? 'src' : 'lib', `${template.id}.${this.features.typescript ? 'ts' : 'js'}`));
            if (dirExist) {
